	     ...
	     loff_t *f_pos)
{
  ...
  if(copy_from_user(&sleep_time_sec, buf, 4))
    {
      return -EINVAL;
    }
  ...
  sleep_time_j = sleep_time_sec * HZ;
  
  time_left = wait_event_interruptible_timeout(wait_queue[minor], flag[minor] != 0, sleep_time_j);
  flag[minor] = 0;
  
  remaining_seconds = time_left / HZ;
